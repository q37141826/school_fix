 ...
 * <p>流式布局</p>
 * <p>根据这个库{https://github.com/nex3z/FlowLayout}修改而来,增加了单选/多选功能.</p>
 */
public class FlowLayout extends ViewGroup implements View.OnClickListener {
    ...
    public FlowLayout(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            mChoiceMode = a.getInt(R.styleable.FlowLayout_chooseMode, CHOICE_MODE_NONE);
        } finally {
            a.recycle();
        }
    }
    ...
    public void addView(View child) {
        super.addView(child);
        addViewInternal(child, getChildCount() - 1);
    }
    ...
    private void addViewInternal(View child, int index) {
        if (mChoiceMode != CHOICE_MODE_NONE && !(child instanceof Checkable)) {
            //可选择模式下，添加的View必须实现Checkable
            throw new IllegalArgumentException("child view must be Checkable");
        }
        //set clickHandler
        child.setOnClickListener(this);
        mChildViews.put(index, child);
        if (mChoiceMode != CHOICE_MODE_NONE) {
            if (mCheckStates == null) {
                mCheckStates = new SparseBooleanArray();
            }
            mCheckStates.put(index, false);
        }
    }

    @Override
    public void onClick(View v) {
        final int position = getViewPosition(v);
        if (mChoiceMode == CHOICE_MODE_NONE) {
        ...
        if (mChoiceMode == CHOICE_MODE_SINGLE) {
            ...
            checkable.toggle();
            mCheckStates.put(position, checkable.isChecked());
        }

        if (onItemSelectedListener != null) {
            onItemSelectedListener.onSelected(v, position);
        }
    }

    private int getViewPosition(View v) {
        return mChildViews.indexOfValue(v);
    }
    ...
    public void checked(int index) {
        View view = mChildViews.get(index);
        if (view != null && (view instanceof Checkable)) {
            clearChecked();
            ((Checkable) view).setChecked(true);
            mCheckStates.put(index, true);
        }
    }
    ...
    public int getSelectedCount() {
        ...
        for (int i = 0; i < mCheckStates.size(); i++) {
            boolean selected = mCheckStates.valueAt(i);
            if (selected) count++;
        }
        return count;
    }
